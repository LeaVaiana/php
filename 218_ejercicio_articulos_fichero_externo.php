<?php
	...
	//B) si no existe el fichero externo, inicializar la variable de sesion
	if(!file_exists('files/articulos.txt')){
		$_SESSION['articulos']=array();
	}else{
	//mover el contenido del fichero a la variable de sesion;
		//leer el fichero
		$articulosFichero = file_get_contents('files/articulos.txt');

		//convertir el texto del fichero (json) a un array
		$_SESSION['articulos']= json_decode($articulosFichero,true);
	...
	$resultado=$descripcion=$precio=$id=$articulo='';
...
//actualizar el fichero con el contenido de la variable de sesion
		//convertir el array de articulos a json
		$articulosFichero = json_encode($_SESSION['articulos']);

		//escribir el fichero
		file_put_contents('files/articulos.txt', $articulosFichero);
